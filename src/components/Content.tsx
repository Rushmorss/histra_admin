...
import { Search, Trash2, MessageSquare, ThumbsUp, MapPin } from "lucide-react";
      ...
      <div className="grid grid-cols-1 gap-6">
        {loading ? (
            Array(6).fill(0).map((_, i) => (
              <div key={i} className="h-[400px] bg-surface-container-low rounded-[2rem] animate-pulse" />
            ))
          ...
          >
            {post.thumbnailUrl && (
              <div className="w-full lg:w-72 h-48 rounded-3xl overflow-hidden bg-surface-container-high border border-outline-variant/10 shrink-0 relative">
                <img src={post.thumbnailUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={post.title} />
                <div className="absolute top-4 left-4 bg-surface/80 backdrop-blur-md px-3 py-1.5 rounded-xl font-label text-[10px] font-black uppercase tracking-widest text-on-surface border border-outline-variant/10 shadow-lg">
                   {post.mediaFiles?.length || 0} Media
                </div>
              </div>
            )}
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex justify-between items-start mb-4">
                 <div className="flex items-center gap-3">
                   <div className="w-10 h-10 rounded-full overflow-hidden bg-surface-container-high border border-outline-variant/20">
                     <img src={post.user?.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(post.user?.fullName || post.user?.username || 'U')}`} className="w-full h-full object-cover" />
                   </div>
                   <div>
                     <p className="font-headline font-bold text-on-surface group-hover:text-primary transition-colors">{post.user?.fullName || post.user?.username}</p>
              ...
              <h3 className="font-display text-2xl font-black text-on-surface mb-3 tracking-tight">{post.title || "No Title"}</h3>
