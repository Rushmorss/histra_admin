    ...
    <header className="fixed top-0 right-0 left-64 h-16 glass z-40 px-8 flex justify-between items-center shadow-sm border-b border-outline-variant/10">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <img
            src={resolveImageUrl(user?.avatarUrl || user?.avatar || user?.profileImage) || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.fullName || user?.username || 'Admin')}&background=6366f1&color=fff`}
            alt="Profile"
